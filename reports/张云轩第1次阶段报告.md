目前词法分析器的输出功能基本实现了，但在枚举类型以及token数据结构上还不符合要求，正在进行更改。尽力在语法分析之前完成该任务，不影响到后续工作。感受比较大的就是合作问题，即使单看自己部分的功能实现了，但在与同学合作的接口上出现问题，也完全不行，因此在以后的学习中，在注重自己任务的同时，更要注重团队协作，不然仅靠自己的工作部分，根本完不成一个完整的项目，这将没有多大意义。
 #include "stdio.h"
 #include "stdlib.h"
 #include "string.h"
 #include "iostream"
